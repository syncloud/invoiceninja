...
def module_setup(request, device, data_dir, platform_data_dir, app_dir, artifact_dir):
    def module_teardown():
        ...
        device.run_ssh('cp -r /var/snap/invoiceninja/current/config {0}/config.current'.format(TMP_DIR), throw=False)
        device.run_ssh('cp -r /snap/invoiceninja/current/config {0}/config.app'.format(TMP_DIR), throw=False)
        device.run_ssh('cp -r /snap/invoiceninja/current/invoiceninja/usr/local/etc/ {0}/usr.local.etc'.format(TMP_DIR), throw=False)
...
def test_storage_change_event(device):
    device.run_ssh('snap run invoiceninja.storage-change > {0}/storage-change.log'.format(TMP_DIR))
